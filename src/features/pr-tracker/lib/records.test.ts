...
describe("checkRecord", () => {
  ...
  it("replaces an entry on a date that already has one, ignoring it as its own neighbour", () => {
    const verdict = checkRecord(line, { weightKg: 105, achievedOn: "2026-09-01" })
    expect(verdict).toMatchObject({ ok: true, placement: "append" })
    if (verdict.ok) {
      expect(verdict.replaces?.id).toBe("b")
      expect(verdict.previous?.id).toBe("a")
    }
  })

  it("still holds the replacement to its remaining neighbours", () => {
    const verdict = checkRecord(line, { weightKg: 95, achievedOn: "2026-09-01" })
    expect(verdict).toMatchObject({ ok: false, reason: "not-heavier" })
  })
