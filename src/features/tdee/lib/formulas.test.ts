...
describe("buildBreakdown", () => {
  const breakdown = buildBreakdown(VALUES, result, "lb")
  ...
  it("notes when a cap kicks in", () => {
    ...
    const cappedBreakdown = buildBreakdown(capped, calculateTdee(toTdeeInput(capped)), "lb")
  ...
  it("shows the kg → lb conversion when targets are in kg", () => {
    const kg = buildBreakdown(VALUES, result, "kg")
...
describe("breakdownToText", () => {
  it("produces a readable plain-text version", () => {
    const text = breakdownToText(buildBreakdown(VALUES, result, "lb"))
